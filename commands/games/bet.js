...
module.exports = class BetCommand extends Command {
	...
	async run(message, { bet, coinsToBet }) {
		const { getDatabase, setDatabase, updateDatabase } = require('../../database/postgres');
		let bCoin = await getDatabase(message.author.id, message.guild.id);
		if (bCoin == undefined) setDatabase(message.author.id, message.guild.id);
		bCoin = await getDatabase(message.author.id, message.guild.id);
